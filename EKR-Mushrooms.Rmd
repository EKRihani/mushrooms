...
\paragraph*{}
The dataset is directly downloaded from the UCI database in zip format, unzipped and the csv file extracted using the *read.csv* function.
\paragraph*{}
The initial and final structures of the dataset are as following :
```{r out.width = "90%", echo = FALSE}
kable(structure_dataset, caption = "Dataset structure")
```
The import automatically converted all character variables to factors. The only modification was to modify some of the 2-level factors into logical variables.
\paragraph*{}
...
```  
\paragraph*{}
The cap diameter distribution looks like a right-skewed bell curve but actually is bimodal, with a main mode toward 5 cm, and a much smaller secondary mode toward 50 cm. This size can look quite surprising, but some species such as *Polyporus squamosus* (dryad's saddle) can be very large and weight up to 5 kg.
\paragraph*{}
The stem height distribution also looks like a right-skewed bell curve, with a main mode around 5 cm, and a secondary mode at 0 cm. Again, this can look surprising, but some mushrooms have no stem, which could explain this height value.
\paragraph*{}
The stem width distribution looks like a right-skewed bell curve, with a peak around 12 mm. In all three distributions, the tail could probably be explained by the impossibility to have negative dimensional values.
