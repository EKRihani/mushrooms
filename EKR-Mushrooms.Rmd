...
```{r, echo = FALSE}
kable(round(fit_ranger_best_results, 6), caption = "Performance of the Ranger model with optimal settings")
```
...
While the model performance is quite honorable, it is not sufficient to fulfill the specificity criterion, which was the primary endpoint of this study.  
...
Species identification is a classic classification task, usually performed by humans using a classification tree strategy. 
\paragraph*{}
This study showed that given particular selection conditions imposed by circumstances (toxic mushrooms shouldn't be classified as edible, i.e. *Specificity* = 1), a significant amount of modelling strategies were giving insufficient results, and proved to be inferior to even the most basic *ad-hoc* bi-criteria classification models.
\paragraph*{}
