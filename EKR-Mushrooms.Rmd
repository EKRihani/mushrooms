...
\paragraph*{}
The code and report were run on the following computer configuration :
...
* R : version 3.6.3
* Packages : tidyverse[@R-tidyverse] (v1.3.1), caret[@-caret] (v6.0-88), ggpubr[@R-ggpubr] (v0.4.0), rmarkdown[@R-rmarkdown] (v2.11), knitr[@R-knitr] (v1.34).
...
\subsubsection{Single and dual criteria classifiers}  
\paragraph*{}
...
\paragraph*{}
Before training, validation and tuning of the algorithms, a training and validation strategy must be defined. Since these models are designed from scratch, and in order to keep the code simple, a basic train-validation split was chosen, with a 90/10 split ratio between the training and validation sets.
\paragraph*{}
...
kable(single_crit_tune, caption = "Single-criterion model margin optimization")
```
...
\paragraph*{}
A second hyperparameter optimization run was performed on the two-criteria model, using this same $m_{1}$ value for all single-criterion classification. This run gave the following results :  
```{r out.width = "90%", echo = FALSE}

kable(dual_crit_tune, caption = "Dual-criterion model margin optimization")
```

The best result was achieved with $m_{1} =$ `r best_margin1["Margin"]` and $m_{2} =$ `r best_margin2["Margin"]` , which a final performance of $Sens_{2} =$ `r best_margin2["Sensitivity"]` for $Spec_{2} =$ `r best_margin2["Specificity"]`.

\newpage
\subsubsection{Caret package models}  
