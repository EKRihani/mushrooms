...
* *Stem width* : in centimeters
* *Stem root* : bulbous (b), swollen (s), club (c), cup (u), equal (e), rhizomorphs (z) or rooted (r)
...
\paragraph*{}
The intial and final structures of the dataset are as follows :
```{r out.width = "90%", echo = FALSE}
kable(structure_dataset, caption = "Dataset structure")
```
Since the number of values of character variables (*Sex*, *ChestPainType*, *RestingECG* and *ST_Slope*) are limited and well-defined, converting them from character to factors makes sense and will simplify some parts of the data analysis. For the same reason, the values *FastingBS*, *ExerciseAngina* and *HeartDisease* are set to a logical class, which both helps the consistency (*FastingBS* and *HeartDisease* use 0/1 levels, whereas *ExerciceAngina* uses Y/N) and interpretability (TRUE/FALSE is easier to interpret than 0/1 levels).
\paragraph*{}
A basic analysis of the numeric variables also shows some incoherent values, such as zero cholesterol or resting blood pressures. These zeroes are obviously missing values, that were converted to *NA*.
\paragraph*{}
We can then get basic information about our population.  
```{r, echo = FALSE, fig.cap = "Age and cholesterol distributions"}
plot(ggarrange(
  plot_distrib_Age,
  plot_distrib_Cholesterol + ylab("")
  )
)
```  
```{r, echo = FALSE, fig.cap = "Resting blood pressure and maximum heart rate distributions"}
plot(ggarrange(
  plot_distrib_RestingBP,
  plot_distrib_MaxHR + ylab("")
  )
)
```
