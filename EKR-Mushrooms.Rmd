...
\paragraph*{}
The primary and secondary *data_meta* files include all information to translate the one-letter variable codes into comprehensible words. However, two variable codes were missing.
\paragraph*{}
Firstly, the *stem.root = f* value wasn't explained. Since most *f* values for the other categorical stem variables (*stem.surface*, *stem.color*) meant the mushroom has no stem, and given the fact that all stem.root = f values are always associated with *stem.surface = f*, *stem.color = f*, *stem.height = 0* and *stem.width = 0*, one can safely assume that *stem.root = f* simply means there is no stem.
...
```{r, echo = FALSE}
kable(fit_Rborist_pred_results[1:5, 1:5], caption = "Performance of the Rborist model (predFixed tuning)")
```
...
```{r, echo = FALSE}
kable(fit_Rborist_best_results, caption = "Performance of the Rborist model (optimal settings)")
```
